"""
EEG Attention Mamba-lite model.
- Preprocess: same as attention_baseline (timestamp labels, z-score, 2s windows with overlap).
- Model: Conv embedding -> Bi-directional selective SSM (Mamba-lite) blocks.
- Outputs: metrics, plots, checkpoint under experiments/attention_mamba_v1.
"""
...
def download_public_dataset() -> Path:
    ...
    if not eeg_data.exists():
        raise FileNotFoundError(f"'EEG Data' not found under {path}")
    return eeg_data


# -----------------------------
# Preprocess (same as attention_baseline)
# -----------------------------
...
# -----------------------------
def parse_args():
    ap = argparse.ArgumentParser(description="Train and evaluate the EEG Attention Mamba-lite model.")
    ...
    ap.add_argument("--bidir", action="store_true", help="Use bidirectional SSM blocks.")
    ...
    ap.add_argument("--save-dir", type=Path, default=Path(__file__).parent / "experiments" / "attention_mamba_v1")
...
def main():
    ...
    data_root = download_public_dataset() if args.download else args.data_root
    ...
    amp = bool(args.use-amp if hasattr(args, 'use-amp') else args.use_amp)  # compatibility
    ...
    lines = [
        "# EEG Attention Mamba-lite Results",
