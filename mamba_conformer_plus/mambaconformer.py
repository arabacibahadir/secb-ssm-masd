...
class SqueezeExcite1d(nn.Module):
    """
    Lightweight channel attention to reweight EEG channels after embedding.
    """
...
def eeg_spec_augment(x: torch.Tensor, time_mask_ratio: float, channel_drop_prob: float):
    """
    Simple SpecAug-like regularization for EEG.
    - time_mask_ratio: fraction of timesteps zeroed once per sample.
    - channel_drop_prob: probability of dropping each channel per sample.
    """
...
def train_one_epoch(model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer,
                    ...
                    time_mask_ratio: float, channel_drop_prob: float):
    model.train()
    total_loss = 0.0
    correct = 0
    total = 0
...
def parse_args():
    ...
    ap.add_argument("--log-every", type=int, default=10, help="Print detailed training metrics every N batches.")
    ap.add_argument("--time-mask-ratio", type=float, default=0.1, help="Fraction of timesteps zeroed during training (SpecAug-like).")
    ap.add_argument("--channel-drop-prob", type=float, default=0.05, help="Probability of zeroing each channel during training.")
